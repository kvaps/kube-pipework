...
_process_container ()
{
    ...
    _pipework_vars="$(docker inspect --format '{{range $index, $val := .Config.Env }}{{printf "%s\"\n" $val}}{{end}}' $c12id \
        ...
        docker inspect --format '{{range $key, $val := .Config.Labels }}{{printf "%s=%s\"\n" $key $val}}{{end}}'  $c12id \
        | grep -e '^annotation\.[^.]*pipework_cmd.*=\|^annotation\.pipework_key=\|annotation.[^.]*pipework_host_route.*=' | sed -e 's/^annotation\./export "/g')"
